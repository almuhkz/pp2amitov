...
from objects import Road, Player, Nitro, Tree, Button, \
					Obstacle, Coins, Fuel
import psycopg2
usern = str(input("Enter your username to start a game: "))
...
pygame.mixer.music.load('Sounds/lil_jon_get_low.mp3')
...
while running:
	...
	cursor.execute("SELECT username FROM user_score WHERE username = ?", (usern,))
	ma = cursor.fetchone()[0]
    	if ma==0:
        	cursor.execute("INSERT INTO user_score VALUES(%s,%s)", (usern,coins,))
    	else:
        cursor.execute("UPDATE user_score SET score = %s WHERE username = %s",(coins,usern))
